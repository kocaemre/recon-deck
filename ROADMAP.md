...
- **v2.5.0** (2026-06-22) — Minor. 🎉 **Opt-in, local-first AI co-pilot**: `Explain` scan output + `Suggest` next commands (KB-grounded, suggest-never-execute) across Ollama / OpenAI / OpenRouter, with a searchable model picker (pricing + FREE badges + per-target cost estimate). **Cross-host attack plan**, **Exam Mode** (one toggle hard-disables AI for OSCP-style exams), opt-in **web-augmented `:online`** CVE/exploit lookups, and **AI usage & cost analytics** at `/settings/usage` (Migration 0023 `ai_usage` ledger — all local SQLite). Stack/version-aware KB conditionals (web 80/443/8080 + version-gated FTP/SSH/SMB/MySQL) and range-aware known-vulns. Findings now render in Markdown/JSON/HTML exports + a `findings-csv` alias. Markdown-rendered AI panels with ordered "game plan" summaries. Ships the beta release channel + curated free-model recommendations. Off by default — nothing leaves your machine unless you opt in.
